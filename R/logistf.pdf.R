logistf.pdf <-
function (x, y, pos, firth = TRUE, weight, control, plcontrol, offset=NULL, b, beta=NULL, loglik=NULL, fit=NULL, old=FALSE, modcontrol)
{
    ...
    if(!is.null(modcontrol$terms.fit)) stop("Please call logistf.pdf with all terms in modcontrol$terms.fit.")
    ...
    if (is.null(fit) & is.null(beta) & is.null(loglik)) {
      ...
      else fit <- logistf.fit(x, y, weight = weight, offset = offset, firth = firth, control = control, modcontrol = modcontrol)
      beta<-fit$coefficients
      loglik<-fit$loglik['full']
    }
...
#       
      ...
      if(old) xx <- logistf.fit.old(x, y, weight = weight, offset = offset,
                ...
                control = control, modcontrol = modcontrol)
#                  )
